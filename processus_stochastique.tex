...
\item Determine $\sigma(X_{1}), \sigma(X_{2})$ and $\sigma(X_{1},X_{2})$ \\[.2em]
	...
	\begin{equation*}
		\sigma(X_{1}) = \{ X_{1}^{-1}(B): B \in \mathbb{R} Borel law \}
	\end{equation*}
	...
	\begin{itemize}
		...
		\end{align*}
		Thus \begin{equation*}
			\sigma(X_{1})=\{ \emptyset, \Omega, \{ HH, HT \}, \{ TT, TH \}, \{ HH, HT, TH \}, \{ HH, TT \}, \{ HT, TH, TT \} \}
		\end{equation*}
...
\item Are $Y_k$ $\mathcal{G}_k$-measurable? \\[0.2em]
Since
$$
...
\item Are $X_k$ $\mathcal{G}_k$-measurable?
\begin{itemize}
    \item For $X_1$:
    $$
...
\item Are $Y_k$ $\mathcal{F}_k$-measurable?
\begin{itemize}
...
\begin{itemize}
    ...
    Therefore
    $$
    \mathbb{E}(X_1\mid X_2=1)=\frac{1}{2}.
    $$
\end{itemize}
